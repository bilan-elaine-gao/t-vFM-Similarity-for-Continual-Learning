...
def validate(val_loader, model, classifier, criterion, opt):
    ...
    return losses.avg, top1.avg, corr, cnt, correct_task/np.sum(cnt)*100.  # loss, val_acc, val_corr, val_cnt, task_acc
...
def main():
    ...
    with open(os.path.join(opt.origin_ckpt, 'acc_buffer_{}.txt'.format(opt.target_task)), 'w') as f:
        ...
        out +='best task accuracy: {:.2f}\n'.format(best_task_acc)
        out += 'val acc: {:.2f} ,task acc: {:.2f}'.format(val_acc, task_acc)
        print(out)
        out += '\n'
