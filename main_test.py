...
#from longtail_dataset import load_places_lt_full_image_dataset
from util import TwoCropTransform, AverageMeter
from util import adjust_learning_rate, warmup_learning_rate
from util import set_optimizer, save_model, load_model
from networks.resnet_big import SupConResNet
from losses_negative_only import SupConLoss,tvMFLoss
from mmd import get_MMD #MK_MMD,
...
def parse_option():
    ...
    opt.model_name = '{}_{}_{}_lr_{}_loss_{}_decay_{}_bsz_{}_temp_{}_trial_{}_{}_{}_{}_{}_{}'.\
        format(opt.dataset, opt.size, opt.model, opt.learning_rate,opt.loss,
...
def set_model(opt):
    ...
    elif opt.loss=='tvMFLoss':
        criterion = tvMFLoss(temperature=opt.temp,kappa=opt.kappa, bias=opt.bias)
...
def train(train_loader, model, model2, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in tqdm(enumerate(train_loader)):
        ...
        if opt.target_task > 0:
            ...
            current_distri=F.normalize(features1_prev_task,dim=1)
            #print('feature size',current_distri.size())
            '''
        ...
        if opt.target_task > 0:
            with torch.no_grad():
                ...
                #feature distribution
                past_distri=F.normalize(features2_prev_task,dim=1)
            ...
            #mk_mmd=MK_MMD(current_distri,past_distri,kernel_mul=2.0, kernel_num=5, fix_sigma=None)  #features1_prev_task
            #mk_mmd=MK_MMD(past_distri,current_distri,kernel_mul=2.0, kernel_num=5, fix_sigma=None)
            mmd_=get_MMD(features2_prev_task,features1_prev_task)
            #loss += mk_mmd mmd_loss
            loss += mmd_
            #mmd_loss.update(mk_mmd.item(), bsz)
            mmd_loss.update(mmd_.item(), bsz)
        ...
        if (idx + 1) % opt.print_freq == 0 or idx+1 == len(train_loader):
            print('Train: [{0}][{1}/{2}]\t'
                  ...
                  'loss {loss.val:.3f} ({loss.avg:.3f} {mmd_loss.avg:.3f})'.format(
                   epoch, idx + 1, len(train_loader), batch_time=batch_time,
                   data_time=data_time, loss=losses, mmd_loss=mmd_loss))
            sys.stdout.flush()
...
def main():
    ...
    for target_task in tqdm(range(0 if opt.resume_target_task is None else opt.resume_target_task+1, opt.end_task)):
